...
# --- LAYOUT DE LA PESTAÑA DE RESUMEN (RESPONSIVO) ---
def get_summary_layout():
    ...
    return html.Div(className="p-2 p-md-4", children=[ # Padding responsivo
        dbc.Row(justify="center", children=[
            # xs=12 (Celular: Ancho completo), lg=8 (PC: Centrado y más angosto)
            dbc.Col(
                dbc.Card(className="shadow-sm", children=[
                    ...
                    dbc.CardBody([
                        html.P(
                            ...
                            "Historial Detallado, Valoración de Inventario, Análisis ABC (Pareto).",
                            className="text-center text-muted mb-4 small"
                        ),
                        html.Hr(),
                        
                        dbc.Row(justify="center", className="mb-4", children=[
                            # CAMBIO 1: Agregamos clases para centrar el contenido (d-flex flex-column align-items-center)
                            dbc.Col([
                                ...
                                dcc.DatePickerRange(
                                    ...
                                    # Quitamos w-100 para que no se estire y se vea centrado compactamente,
                                    # o lo dejamos si prefieres que ocupe todo el ancho disponible de la columna central.
                                    # Lo dejaremos con w-auto o style específico si quieres que se vea "en medio" sin estirarse.
                                    # Pero para asegurar centrado visual, align-items-center es la clave.
                                    className="" 
                                ),
                                # Switch estético
                                dbc.Switch(
                        ...
                        # Botón ancho completo para fácil acceso táctil
                        html.Div(
                            dbc.Button(
                ...
                xs=12, md=10, lg=6 # Control del ancho de la tarjeta central
            )
        ])
    ])
...
def create_dashboard_sheet(writer, user_id, start_date=None, end_date=None):
    ...
    if not expenses_df.empty:
        ...
        if not exp_cat_df.empty:
            ...
            df_top_exp = pd.DataFrame(); title_exp = "Top Gastos"
    else:
        df_top_exp = pd.DataFrame(); title_exp = "Top Gastos"
...
def generate_excel_summary(user_id, start_date=None, end_date=None):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # CAMBIO 2: Mover Pareto (ABC) al final de todo
